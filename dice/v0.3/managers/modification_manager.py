...
from ui import display_error, display_warning, display_success, display_message
...
class ModificationManager:
    ...
    def _main_menu(self):
        ...
        while True:
            print()
            display_message(f"Player gold: {self.player.gold}")
            display_message(self.current_session.die.str_all())
    ...
    def _upgrade_die_flow(self):
        ...
        display_message(f"{die} has {die.upgrades} upgrades. Next upgrade will cost {cost}.")
        if self.player.gold < cost:
            display_warning("Not enough money")
            return ModificationResult.FAILURE
        ...
        if choice == 'y':
            ...
            die.upgrade()
            display_message("Succesfully upgraded die")
            return ModificationResult.SUCCESS
    ...
    def _select_rune(self, runes):
        ...
        return select_from_list(options, "Availible runes: ", "Choose rune ('Enter' to exit): ")

    def _select_face(self, die):
        options = [str(r) for r in die.runes]
        return select_from_list(options, f"{die} faces:", "Choose face ('Enter' to exit): ")